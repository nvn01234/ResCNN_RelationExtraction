...
def test(testing_data, input_x, input_p1, input_p2, s, p, dropout_keep_prob, datamanager, sess, num_epoch):
    ...
    for i in range(len(testing_data)):
        results.append(("", testing_data[i].relation.id, max_pro, prediction))
        if testing_data[i].relation.id == prediction and prediction != 0:
            c += 1
        t += 1
    ...
    for i in range(total):
        ...
        if i % 100 == 0:
            print(
                "Precision: " + str(float(correct) / float(i + 1)) + "  Recall: " + str(float(correct) / float(total)))
        f.write(str(float(correct) / float(i + 1)) + "    " + str(float(correct) / float(total)) + "    " + str(
            results[i][2]) + "  "
